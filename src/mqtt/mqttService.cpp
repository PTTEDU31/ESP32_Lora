...
bool MqttService::writeToMqtt(DataMessage *message)
{
    ...
    mqttMessageSend->topic = String(firmwareOptions.mqtt_topic_out) + String(message->addrSrc);
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
    {
        ...
        mqtt_connected = true;
        String topic = String(firmwareOptions.mqtt_topic_sub) + MqttService::getInstance().localName;
...
{
    if (strlen(firmwareOptions.mqtt_server) == 0)
    {
        ESP_LOGE(MQTT_TAG, "MQTT server address is not set!");
        return;
    }

    char uri[128];
    snprintf(uri, sizeof(uri), "mqtt://%s:%d", firmwareOptions.mqtt_server, firmwareOptions.mqtt_port);

    ESP_LOGI(MQTT_TAG, "MQTT URI: %s", uri);

    esp_mqtt_client_config_t mqtt_cfg = {};
    mqtt_cfg.broker.address.uri = uri;
    mqtt_cfg.credentials.client_id = client_id;
    mqtt_cfg.credentials.username = firmwareOptions.mqtt_username[0] ? firmwareOptions.mqtt_username : nullptr;
    mqtt_cfg.credentials.authentication.password = firmwareOptions.mqtt_password[0] ? firmwareOptions.mqtt_password : nullptr;

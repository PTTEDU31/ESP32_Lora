...
void i2s_init(void)
{
	...
	i2s_event_callbacks_t cbs = {
        .on_recv = NULL,
        .on_recv_q_ovf = NULL,
        .on_sent = i2s_tx_queue_overflow_callback,
...
void send_595(){
	uint8_t *w_buf = (uint8_t *)calloc(1, 1029);
	...
	ESP_LOGW(TAG, "Writing data...");
	ESP_LOGD("Tung","LOGD");

	for (int i = 0; i < 1; i++) {
		ESP_ERROR_CHECK(i2s_channel_write(tx_chan, w_buf, 1029, &w_bytes, 1000));
	}

	// Thêm một độ trễ nhỏ để đảm bảo log được in ra
